import { Notes, Head, Steps } from "mdx-deck";
import { Meta, Profile, AlignLeft, Header } from "./parts";
import { CodeSurferLayout, vsDarkPartial } from "code-surfer";
import "main.css";

export const theme = vsDarkPartial;

<Head>
  <Meta
    title="CDKのコードの読み方(仮)"
    description="cdk"
    publishedAt={"2022-03-04T15:00:51.470Z"}
    host=""
  />
</Head>


# CDK のコードの読み方(仮)

[AWS CDK Conference Japan](https://jawsug.connpass.com/event/240422/)
@yamatatsu

---

<AlignLeft>

<Header>About Me</Header>

<Profile />

</AlignLeft>

---

## 僕が CDK にコントリビュートしたこと

---

<Header>僕が CDK にコントリビュートしたこと</Header>

- aws-iot
- aws-iotevents

---

<CodeSurferLayout>

```ts
import * as iot from 'aws-cdk-lib/aws-iot';

const firehoseActionRole = new iam.Role(this, 'firehoseActionRole', {
  assumedBy: new iam.ServicePrincipal('iot.amazonaws.com'),
});
stream.grantPutRecords(firehoseActionRole);

const s3ActionRole = new iam.Role(this, 's3ActionRole', {
  assumedBy: new iam.ServicePrincipal('iot.amazonaws.com'),
});
bucket.grantPut(s3ActionRole);

new iot.CfnTopicRule(this, 'MyTopicRule' {
  topicRulePayload: {
    sql: "SELECT * FROM 'things/+/data'",
    actions: [
      {
        lambda: {
          functionArn: func.functionArn,
        },
      },
      {
        firehose: {
          deliveryStreamName: stream.deliveryStreamName,
          roleArn: firehoseActionRole.roleArn,
        },
      },
    ],
    errorAction: {
      s3: {
        bucketName: bucket.bucketName,
        key: 'key',
        roleArn: s3ActionRole.roleArn,
      },
    },
  },
});
```

```ts 1
import * as iot from 'aws-cdk-lib/aws-iot';

const firehoseActionRole = new iam.Role(this, 'firehoseActionRole', {
  assumedBy: new iam.ServicePrincipal('iot.amazonaws.com'),
});
stream.grantPutRecords(firehoseActionRole);

const s3ActionRole = new iam.Role(this, 's3ActionRole', {
  assumedBy: new iam.ServicePrincipal('iot.amazonaws.com'),
});
bucket.grantPut(s3ActionRole);

new iot.CfnTopicRule(this, 'MyTopicRule' {
  topicRulePayload: {
    sql: "SELECT * FROM 'things/+/data'",
    actions: [
      {
        lambda: {
          functionArn: func.functionArn,
        },
      },
      {
        firehose: {
          deliveryStreamName: stream.deliveryStreamName,
          roleArn: firehoseActionRole.roleArn,
        },
      },
    ],
    errorAction: {
      s3: {
        bucketName: bucket.bucketName,
        key: 'key',
        roleArn: s3ActionRole.roleArn,
      },
    },
  },
});
```

```ts
import * as iot from '@aws-cdk/aws-iot-alpha';
import * as actions from '@aws-cdk/aws-iot-actions-alpha';

const firehoseActionRole = new iam.Role(this, 'firehoseActionRole', {
  assumedBy: new iam.ServicePrincipal('iot.amazonaws.com'),
});
stream.grantPutRecords(firehoseActionRole);

const s3ActionRole = new iam.Role(this, 's3ActionRole', {
  assumedBy: new iam.ServicePrincipal('iot.amazonaws.com'),
});
bucket.grantPut(s3ActionRole);

new iot.CfnTopicRule(this, 'MyTopicRule' {
  topicRulePayload: {
    sql: "SELECT * FROM 'things/+/data'",
    actions: [
      {
        lambda: {
          functionArn: func.functionArn,
        },
      },
      {
        firehose: {
          deliveryStreamName: stream.deliveryStreamName,
          roleArn: firehoseActionRole.roleArn,
        },
      },
    ],
    errorAction: {
      s3: {
        bucketName: bucket.bucketName,
        key: 'key',
        roleArn: s3ActionRole.roleArn,
      },
    },
  },
});
```

```ts 4:12
import * as iot from '@aws-cdk/aws-iot-alpha';
import * as actions from '@aws-cdk/aws-iot-actions-alpha';

const firehoseActionRole = new iam.Role(this, 'firehoseActionRole', {
  assumedBy: new iam.ServicePrincipal('iot.amazonaws.com'),
});
stream.grantPutRecords(firehoseActionRole);

const s3ActionRole = new iam.Role(this, 's3ActionRole', {
  assumedBy: new iam.ServicePrincipal('iot.amazonaws.com'),
});
bucket.grantPut(s3ActionRole);

new iot.CfnTopicRule(this, 'MyTopicRule' {
  topicRulePayload: {
    sql: "SELECT * FROM 'things/+/data'",
    actions: [
      {
        lambda: {
          functionArn: func.functionArn,
        },
      },
      {
        firehose: {
          deliveryStreamName: stream.deliveryStreamName,
          roleArn: firehoseActionRole.roleArn,
        },
      },
    ],
    errorAction: {
      s3: {
        bucketName: bucket.bucketName,
        key: 'key',
        roleArn: s3ActionRole.roleArn,
      },
    },
  },
});
```

```ts
import * as iot from '@aws-cdk/aws-iot-alpha';
import * as actions from '@aws-cdk/aws-iot-actions-alpha';

new iot.CfnTopicRule(this, 'MyTopicRule' {
  topicRulePayload: {
    sql: "SELECT * FROM 'things/+/data'",
    actions: [
      {
        lambda: {
          functionArn: func.functionArn,
        },
      },
      {
        firehose: {
          deliveryStreamName: stream.deliveryStreamName,
          roleArn: firehoseActionRole.roleArn,
        },
      },
    ],
    errorAction: {
      s3: {
        bucketName: bucket.bucketName,
        key: 'key',
        roleArn: s3ActionRole.roleArn,
      },
    },
  },
});
```


```ts 4:28
import * as iot from '@aws-cdk/aws-iot-alpha';
import * as actions from '@aws-cdk/aws-iot-actions-alpha';

new iot.CfnTopicRule(this, 'MyTopicRule' {
  topicRulePayload: {
    sql: "SELECT * FROM 'things/+/data'",
    actions: [
      {
        lambda: {
          functionArn: func.functionArn,
        },
      },
      {
        firehose: {
          deliveryStreamName: stream.deliveryStreamName,
          roleArn: firehoseActionRole.roleArn,
        },
      },
    ],
    errorAction: {
      s3: {
        bucketName: bucket.bucketName,
        key: 'key',
        roleArn: s3ActionRole.roleArn,
      },
    },
  },
});
```

```ts
import * as iot from '@aws-cdk/aws-iot-alpha';
import * as actions from '@aws-cdk/aws-iot-actions-alpha';

new iot.TopicRule(this, 'MyTopicRule' {
  sql: iot.IotSql.fromStringAsVer20160323("SELECT * FROM 'things/+/data'"),
  actions: [
    new actions.FirehoseStreamAction(stream),
    new actions.LambdaFunctionAction(func),
  ],
  errorAction: new actions.S3PutObjectAction(bucket)
});
```

```ts 1:11
import * as iot from '@aws-cdk/aws-iot-alpha';
import * as actions from '@aws-cdk/aws-iot-actions-alpha';

new iot.TopicRule(this, 'MyTopicRule' {
  sql: iot.IotSql.fromStringAsVer20160323("SELECT * FROM 'things/+/data'"),
  actions: [
    new actions.FirehoseStreamAction(stream),
    new actions.LambdaFunctionAction(func),
  ],
  errorAction: new actions.S3PutObjectAction(bucket)
});
```

</CodeSurferLayout>

---


<CodeSurferLayout>

```ts
const role = new iam.Role(this, 'MyRole', {});

new iotevents.CfnDetectorModel(this, 'HeartBeat', {
  roleArn: role.roleArn,
  detectorModelDefinition: {
    initialStateName: 'Normal',
    states: [
      {
        stateName: 'Normal',
        onEnter: {
          events: [{
            eventName: 'Create_timer',
            condition: 'currentInput("AWS_IoTEvents_Blueprints_Heartbeat_Input")',
            actions: [{
              setTimer: { seconds: 300, timerName: 'awake' },
            }],
          }],
        },
        onInput: {
          transitionEvents: [{
            eventName: 'Go_offline',
            condition: 'timeout("awake")',
            nextState: 'Offline',
          }],
          events: [{
            eventName: 'Reset_timer',
            condition: 'currentInput("AWS_IoTEvents_Blueprints_Heartbeat_Input")',
            actions: [{
              resetTimer: { timerName: 'awake' },
            }],
          }],
        },
      },
      {
        stateName: 'Offline',
        onEnter: {
          events: [{
            eventName: 'Send_notification',
            actions: [{
              sns: { targetArn: 'sns-topic-arn' },
            }],
          }],
        },
        onInput: {
          transitionEvents: [{
            eventName: 'To_normal',
            condition: 'currentInput("AWS_IoTEvents_Blueprints_Heartbeat_Input")',
            nextState: 'Normal',
          }],
        },
      },
    ],
  },
});
```

```ts
const normalState = new iotevents.State({
  stateName: 'Normal',
  onEnter: [{
    eventName: 'Create_timer',
    condition: iotevents.Expression.currentInput(input),
    actions: [new SetTimer('awake', { seconds: 300 })],
  }],
  onInput: [{
    eventName: 'Reset_timer',
    condition: iotevents.Expression.currentInput(input),
    actions: [new ResetTimer('awake')],
  }],
});
const offlineState = new iotevents.State({
  stateName: 'Offline',
  onEnter: [{
    eventName: 'Send_notification',
    actions: [new SnsPublishTopicAction(topic)],
  }],
});

normalState.transitionTo(offlineState, {
  when: iotevents.Expression.timeout('awake'),
})
offlineState.transitionTo(normalState, {
  when: iotevents.Expression.currentInput(input),
})

new iotevents.DetectorModel(this, 'HeartBeat', {
  initialState: normalState,
});
```

</CodeSurferLayout>

---

`aws-iotevents` の方はまだ工事中で使えません

<img src={require("file-loader!./parts/assets/ojigi_animal_inu.png")} height="120" />

---

<AlignLeft>

<Header>概要</Header>

1. ゴール
1. CDK のコードの中
1. コントリビュートに向けて
1. おわりに

</AlignLeft>

---

## ゴール

---

<Header>ゴール</Header>

### 😩 → 🤩

---

<Header>ゴール</Header>

### ドキュメントが足りない。

### 😩 ぐぬぬ

---

<Header>ゴール</Header>

### コード読めばいいや

### 😇

---

<Header>ゴール</Header>

### Props の属性が足りない。機能が足りない。

### 😩 ぐぬぬ

---

<Header>ゴール</Header>

### コントリビューションチャンス！

### 🤩

---

<Header>ゴール</Header>

# 😩 → 🤩

---

## CDK のコードの中

---

<Header>CDK のコードの中</Header>

`aws-lambda/lib/function.ts`  
の中を見てみよう :eyes:

---

<CodeSurferLayout>

```ts subtitle="このファイルにはFunctionクラスが定義されています"
export class Function extends cdk.Resource {
}
```

```ts 1[31:43] subtitle="ResourceはAWSリソースを抽象するクラスです"
export class Function extends cdk.Resource {
}
```

```ts subtitle="クラスにはconstructorが定義されています"
export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {
  }
}
```

```ts 2[15:65] subtitle="Constructを継承するクラスはconstructorにて必ず3つの引数をとります"
export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {
  }
}
```

```ts 2[52:65] subtitle="3つめにはクラスごとに独自のProps型をとります"
export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {
  }
}
```

```ts subtitle="ほとんどの場合、同じファイル内に定義されています"
export interface FunctionProps {
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {
  }
}
```

```ts subtitle="おなじみのcodeもここで定義されています"
export interface FunctionProps {
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {
  }
}
```

```ts subtitle="Propsの属性には必ずjsdocが書かれています"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {
  }
}
```

```ts 2:6 subtitle="ちなみにjsdocがないとCIが落ちるので最近手が入ったコードは必ずjsdocがあります😅"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {
  }
}
```

```ts 2:6 subtitle="ときどき記載内容が間違ってたりします"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {
  }
}
```

```ts 2:6 subtitle="チャンスです🤩"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {
  }
}
```

```ts subtitle="propsはconstructorの中で使われます"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {

    const code = props.code;

    const resource = new CfnFunction(this, 'Resource', {
      code: {
        s3Bucket: code.s3Location && code.s3Location.bucketName,
        s3Key: code.s3Location && code.s3Location.objectKey,
        s3ObjectVersion: code.s3Location && code.s3Location.objectVersion,
        zipFile: code.inlineCode,
        imageUri: code.image?.imageUri,
      },
    });
  }
}
```

```ts 15:23 subtitle="constructorの中にはCfnxxxというクラスが使われています"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {

    const code = props.code;

    const resource = new CfnFunction(this, 'Resource', {
      code: {
        s3Bucket: code.s3Location && code.s3Location.bucketName,
        s3Key: code.s3Location && code.s3Location.objectKey,
        s3ObjectVersion: code.s3Location && code.s3Location.objectVersion,
        zipFile: code.inlineCode,
        imageUri: code.image?.imageUri,
      },
    });
  }
}
```

```ts 15:23 subtitle="これはL1クラスと呼ばれ、CloudFormationのインターフェースをそのまま実装したクラスです"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {

    const code = props.code;

    const resource = new CfnFunction(this, 'Resource', {
      code: {
        s3Bucket: code.s3Location && code.s3Location.bucketName,
        s3Key: code.s3Location && code.s3Location.objectKey,
        s3ObjectVersion: code.s3Location && code.s3Location.objectVersion,
        zipFile: code.inlineCode,
        imageUri: code.image?.imageUri,
      },
    });
  }
}
```

```ts 15:23 subtitle="なのでCloudFormationのドキュメントを読めば使い方がわかります"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {

    const code = props.code;

    const resource = new CfnFunction(this, 'Resource', {
      code: {
        s3Bucket: code.s3Location && code.s3Location.bucketName,
        s3Key: code.s3Location && code.s3Location.objectKey,
        s3ObjectVersion: code.s3Location && code.s3Location.objectVersion,
        zipFile: code.inlineCode,
        imageUri: code.image?.imageUri,
      },
    });
  }
}
```

```ts 1:25 subtitle="これはかなり単純化したコードではありますが、基本的にはこんな感じです"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {

    const code = props.code;

    const resource = new CfnFunction(this, 'Resource', {
      code: {
        s3Bucket: code.s3Location && code.s3Location.bucketName,
        s3Key: code.s3Location && code.s3Location.objectKey,
        s3ObjectVersion: code.s3Location && code.s3Location.objectVersion,
        zipFile: code.inlineCode,
        imageUri: code.image?.imageUri,
      },
    });
  }
}
```

```ts 1:25 subtitle="入力(Props)と出力(Cfn)の仕組みが分かると、読める気がしてきませんか？"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {

    const code = props.code;

    const resource = new CfnFunction(this, 'Resource', {
      code: {
        s3Bucket: code.s3Location && code.s3Location.bucketName,
        s3Key: code.s3Location && code.s3Location.objectKey,
        s3ObjectVersion: code.s3Location && code.s3Location.objectVersion,
        zipFile: code.inlineCode,
        imageUri: code.image?.imageUri,
      },
    });
  }
}
```

```ts 1:25 subtitle="それではこのクラスにhandlerを追加する機能追加してみましょう"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {

    const code = props.code;

    const resource = new CfnFunction(this, 'Resource', {
      code: {
        s3Bucket: code.s3Location && code.s3Location.bucketName,
        s3Key: code.s3Location && code.s3Location.objectKey,
        s3ObjectVersion: code.s3Location && code.s3Location.objectVersion,
        zipFile: code.inlineCode,
        imageUri: code.image?.imageUri,
      },
    });
  }
}
```

```ts subtitle="まずはCloudFormationのドキュメントに従ってhandlerをL1クラスに渡します"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {

    const code = props.code;

    const resource = new CfnFunction(this, 'Resource', {
      code: {
        s3Bucket: code.s3Location && code.s3Location.bucketName,
        s3Key: code.s3Location && code.s3Location.objectKey,
        s3ObjectVersion: code.s3Location && code.s3Location.objectVersion,
        zipFile: code.inlineCode,
        imageUri: code.image?.imageUri,
      },
      handler: props.handler,
    });
  }
}
```

```ts subtitle="次に、Propsにhandlerを生やしてあげます"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;

  readonly handler: string;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {

    const code = props.code;

    const resource = new CfnFunction(this, 'Resource', {
      code: {
        s3Bucket: code.s3Location && code.s3Location.bucketName,
        s3Key: code.s3Location && code.s3Location.objectKey,
        s3ObjectVersion: code.s3Location && code.s3Location.objectVersion,
        zipFile: code.inlineCode,
        imageUri: code.image?.imageUri,
      },
      handler: props.handler,
    });
  }
}
```

```ts subtitle="最後に、CloudFormationのドキュメントなどを参考にjsdocを書きます"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;

  /**
   * The name of the method within your code that Lambda calls to execute
   * your function. The format includes the file name. It can also include
   * namespaces and other qualifiers, depending on the runtime.
   */
  readonly handler: string;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {

    const code = props.code;

    const resource = new CfnFunction(this, 'Resource', {
      code: {
        s3Bucket: code.s3Location && code.s3Location.bucketName,
        s3Key: code.s3Location && code.s3Location.objectKey,
        s3ObjectVersion: code.s3Location && code.s3Location.objectVersion,
        zipFile: code.inlineCode,
        imageUri: code.image?.imageUri,
      },
      handler: props.handler,
    });
  }
}
```

```ts 1:30 subtitle="あとはテストとREADMEを追記すればPRを出せます！"
export interface FunctionProps {
  /**
   * The source code of your Lambda function. You can point to a file in an
   * Amazon Simple Storage Service (Amazon S3) bucket or specify your source
   * code as inline text.
   */
  readonly code: Code;

  /**
   * The name of the method within your code that Lambda calls to execute
   * your function. The format includes the file name. It can also include
   * namespaces and other qualifiers, depending on the runtime.
   */
  readonly handler: string;
}

export class Function extends cdk.Resource {
  constructor(scope: Construct, id: string, props: FunctionProps) {

    const code = props.code;

    const resource = new CfnFunction(this, 'Resource', {
      code: {
        s3Bucket: code.s3Location && code.s3Location.bucketName,
        s3Key: code.s3Location && code.s3Location.objectKey,
        s3ObjectVersion: code.s3Location && code.s3Location.objectVersion,
        zipFile: code.inlineCode,
        imageUri: code.image?.imageUri,
      },
      handler: props.handler,
    });
  }
}
```

</CodeSurferLayout>

---

<Header>CDK のコードの中</Header>

今回のコードは実物をかなり単純化したものですが、基本的な読み方に変わりはありません。（例外：Custom ResourceやAssetsなど）

ドキュメントの不備や、生えてない機能を見つけたらコントリビュートしてみてはいかがでしょうか？

---

## コントリビュートに向けて

---

<Header>コントリビュートに向けて</Header>

- Unit Testを書く
- Integ Testを書く
- READMEを書く
- PRを作る
- CIを通す

TODO: それぞれ書く

---

## おわりに

---

ご清澄ありがとうございました :pray:
